...
async function requireRefreshTokenForEmail(email: string) {
...
async function listMarketingEvents(
  refreshToken: string
): Promise<EventProspectPair[]> {
  const events = await listUpcomingEventsForUser(
    refreshToken,
    CALENDAR_LOOKAHEAD_LIMIT
  );
  ...
  const pairs = events
    ...
    .filter(
      (pair): pair is EventProspectPair =>
        Boolean(pair.prospect && pair.prospect.startTime)
    )
...
async function getEventForUser(
  refreshToken: string,
  eventId: string
 ...
 */
app.get("/preps", async (req: Request, res: Response) => {
  try {
    ...
    const limited = events
      ...
      .sort(
        (a, b) =>
          new Date(a.prospect.startTime).getTime() -
    ...
    if ((err as { code?: string }).code === "MISSING_EMAIL") {
    ...
    if ((err as { code?: string }).code === "NO_REFRESH_TOKEN") {
      ...
      return res.json({
        meetings: [],
        recommendation:
          "We couldn’t access your Google Calendar. Please re‑sign in and accept the calendar permission.",
      });
    }
 ...
 */
app.get("/meetings/:eventId/prep", async (req: Request, res: Response) => {
  try {
    const email = requireUserEmail(req);
    const refreshToken = await requireRefreshTokenForEmail(email);
    ...
    const { prep, context } = await getOrCreateMeetingPrepForEvent(event);
    const startTime =
      context.prospect.startTime || event.start?.dateTime || "";
    ...
    if ((err as { code?: string }).code === "NO_REFRESH_TOKEN") {
      return res.status(403).json({
        error:
          "Google Calendar access not granted for this user. Please re‑connect.",
      });
    }
...
app.get("/debug/prep", async (_req: Request, res: Response) => {
  try {
    ...
    const context = await buildResearchContext(prospect as any);
    console.log("ResearchContext built");

    if (!(context as any).isAcceptableMeeting) {
      return res.status(200).json({
        message:
          "No qualifying sales/marketing meeting scheduled for this event based on web profile.",
        prospect: context.prospect,
        person: (context as any).person,
        company: (context as any).company,
      });
    }

    const prep = await generateMeetingPrep(context as any);
