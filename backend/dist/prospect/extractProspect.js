...
function domainToCompany(domain) {
    ...
    if (PERSONAL_DOMAINS.has(lower))
        return undefined;
    const mainPart = lower.split(".")[0] || lower;
    return mainPart.charAt(0).toUpperCase() + mainPart.slice(1);
}
function guessCompanyFromTitle(title, fallback) {
    if (!title)
        return fallback;
...
function guessRoleFromText(title, description) {
...
export function extractProspect(event) {
    const attendees = event.attendees ?? [];
    const primary = attendees.find((a) => !a.self) ?? attendees[0] ?? undefined;
    const email = primary?.email ?? undefined;
    const emailDomain = email ? email.split("@")[1] : undefined;
    const fullName = cleanName(primary?.displayName) ||
        cleanName(email) ||
        "Unknown";
    ...
    const companyNameGuess = guessCompanyFromTitle(meetingTitle, companyFromDomain);
    const roleGuess = guessRoleFromText(meetingTitle, meetingDescription);
    return {
        eventId: event.id ?? "",
        fullName,
        email,
...
//# sourceMappingURL=extractProspect.js.map