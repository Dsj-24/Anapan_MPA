...
function normSection(val) {
    ...
    return val
        ...
        if (item && typeof item === "object") {
            ...
            const hasTitleField = typeof titleField === "string" && titleField.trim().length > 0;
            const hasDetailField = typeof detailField === "string" && detailField.trim().length > 0;
            if (hasTitleField || hasDetailField) {
                const title = hasTitleField ? titleField.trim() : "";
                const detail = hasDetailField ? detailField.trim() : "";
                if (title && detail)
                    return `${title}: ${detail}`;
                if (title)
                    return title;
...
export async function generateMeetingPrep(context) {
    ...
    const safe = {
        ...
        toneSummary: typeof parsed?.toneSummary === "string" ? parsed.toneSummary : "",
    };
    return MeetingPrepSchema.parse(safe);
}
//# sourceMappingURL=generateMeetPrep.js.map